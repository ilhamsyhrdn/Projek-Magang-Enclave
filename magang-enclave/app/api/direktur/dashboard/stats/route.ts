...
export async function GET(request: NextRequest) {
  try {
    ...
    const suratMasukQuery = `
      ...
      LEFT JOIN incoming_mail_approvals ima ON im.id = ima.incoming_mail_id
    ...
    const suratKeluarQuery = `
      ...
      LEFT JOIN outgoing_mail_approvals oma ON om.id = oma.outgoing_mail_id
    ...
    const memoQuery = `
      SELECT 
        ...
        COUNT(*) as total_count
      FROM memos m
      LEFT JOIN memo_approval_flows maf ON m.id = maf.memo_id
    ...
    const stats = {
      suratMasuk: {
        created: parseInt(suratMasukResult[0]?.pending_approval || '0'),
        total: parseInt(suratMasukResult[0]?.total_count || '0'),
      },
      suratKeluar: {
        created: parseInt(suratKeluarResult[0]?.pending_approval || '0'),
        total: parseInt(suratKeluarResult[0]?.total_count || '0'),
      },
      memo: {
        created: parseInt(memoResult[0]?.pending_approval || '0'),
