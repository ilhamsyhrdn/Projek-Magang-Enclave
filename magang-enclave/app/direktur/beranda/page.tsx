...
export default function BerandaApproverPage() {
  useRequireAuth(['director']);
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch statistics
        try {
          const statsResponse = await fetch('/api/direktur/dashboard/stats', {
            headers: {
        ...
        try {
          const priorityResponse = await fetch('/api/direktur/dashboard/priority-letters', {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
  ...
  const handleLetterClick = (letter: any) => {
    const routes: Record<string, string> = {
      'incoming_mail': '/direktur/surat-masuk',
      'outgoing_mail': '/direktur/surat-keluar',
      'memo': '/direktur/memo',
      'notulensi': '/direktur/notulensi',
    };
